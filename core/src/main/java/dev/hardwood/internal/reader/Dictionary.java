...
public sealed interface Dictionary {
    ...
    /// Decode dictionary values into a Page for the index-only fused path
    /// (required dictionary columns, `maxDefinitionLevel == 0`). No def-level
    /// cursor exists; only the index cursor and the dictionary are carried.
    default Page decodePageIndexOnly(HybridStreamCursor indexCursor, int numValues,
                                     int maxDefLevel) {
        return switch (this) {
            case IntDictionary dict -> new Page.IntPage(null, null, null, maxDefLevel, numValues, dict, null, indexCursor, 0);
            case LongDictionary dict -> new Page.LongPage(null, null, null, maxDefLevel, numValues, dict, null, indexCursor, 0);
            case FloatDictionary dict -> new Page.FloatPage(null, null, null, maxDefLevel, numValues, dict, null, indexCursor, 0);
            case DoubleDictionary dict -> new Page.DoublePage(null, null, null, maxDefLevel, numValues, dict, null, indexCursor, 0);
            case ByteArrayDictionary dict -> new Page.ByteArrayPage(null, null, null, maxDefLevel, numValues, dict, null, null, indexCursor, 0);
        };
    }
