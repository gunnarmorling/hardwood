...
- `ParquetFileReader.open(InputFile, HardwoodContext, FileMetaData)` opens a reader from a footer the caller has already parsed, so opening a reader over an unchanging file no longer has to re-read and re-parse its footer. The footer comes from `getFileMetaData()`, which stays usable after its reader is closed.
