...
- A logical type annotation a column's physical type cannot carry is now dropped, and the column read as its physical type, rather than surfacing as an `IllegalArgumentException` from whichever accessor first reached it ([#1139](https://github.com/hardwood-hq/hardwood/issues/1139)). Being exactly two bytes wide is the whole definition of `FLOAT16`, so a column declaring three has said two things that cannot both be true; the format's answer, adopted in [parquet-format PR 606](https://github.com/apache/parquet-format/pull/606), is to ignore the annotation and use only the physical type. An annotation this version does not recognize at all is dropped the same way, which is what lets a file written against a newer format version still be read. Both log a warning naming the column. **What changes for you:** `getFileSchema()` reports no logical type for such a column, `getValue` returns its physical value where it used to throw, and a logical accessor on it now fails the way it does on any unannotated column.
- Exceptions raised for a mistake in the calling code no longer carry the `[fileName]` prefix. A column outside the projection, a primitive accessor on a null field, a `ColumnReader` used before its first batch — none of these is a fault of the file, and the remedy is the same whichever file the reader is on. Failures that *are* the file's still name it. Only message text changes; the exception types are unaffected.
