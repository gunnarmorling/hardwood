...
- **Comparison predicates apply to leaf columns only.** `isNull` and `isNotNull` also support
  non-repeated struct groups, so `isNull("address")` tests whether the `address` struct itself is
  null. A struct that is present but whose child fields are all null is considered non-null.
  `LIST`, `MAP`, groups below repeated paths, and comparison predicates on groups remain
  unsupported and are rejected with `IllegalArgumentException`.
