...
The footer's key-value metadata and its `created_by` identifier describe the file rather than any row group, so they are set on the writer, at any point until `close()`:
