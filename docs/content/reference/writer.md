...
Every option here governs a row group and the pages within it. The footer's file-scope fields — the key-value metadata and the `created_by` identifier — are set on the writer instead, and are described under [File Metadata](#file-metadata).
...
The footer carries two fields that describe the file rather than any row group. Both are set on the `ParquetFileWriter` itself, at any point until `close()` writes the footer.
