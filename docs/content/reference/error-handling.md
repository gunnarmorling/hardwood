...
Every message names the file the reader was on. How much *more* it names is the tell: a
failure that is the file's fault carries the read position with it — the row group and page
it stopped at — because that is what varies from one to the next. A mistake in your call
does not vary between row groups or pages, so the file and the column are all there is to
say.
...
These are mistakes in the calling code rather than read failures. The message names the
file and the column, and nothing more: the file is not what is wrong, and the fix is the
same wherever in it the reader had got to.
...
| `ClassCastException` | Calling an accessor that does not fit the column — `getLong` on an `INT32` column. The type is unspecified and may change; see [Type mismatches](accessors.md#type-mismatches). Today it surfaces from the cast of the column's storage array |
