...
into something the build checks. Increments 32–36 are one thread rather than five: each closes
...
36 belongs to the same thread and is the one entry in it that is not an API question: the
shape is expressible in the input contract and readable by the reader, and only the shredder
refuses it, so a file the reader accepts cannot be written back.
