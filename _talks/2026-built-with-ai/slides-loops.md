...
<span class="aside">Everyone spent the holidays with the new models. Software was solved.</span>
...
<object class="chart" type="image/svg+xml" data="images/timeline-start.svg" aria-label="Timeline, start of the year: Jan 4 where is the disconnect, 1,069 likes; Jan 6 Hardwood started"></object>

Note:
Only the first week of the year is filled in. The rest of the axis stays empty
on purpose: the full version comes back at the end.

Data: timeline/posts.tsv. Regenerate with timeline/make_timeline.py.

---

<img class="post" src="images/x-2026-01-04-where-is-the-disconnect.png" alt="Jan 4: Trying to reconcile the Claude did in 1h what took a Google team a year thing with my own LLM experiences">

Note:
The frenzy, and my reaction to it: "Claude did in 1h what took a Google team a
year" against code that leaks resources and has data races.

1,069 likes. I wasn't the only one who noticed the gap.
...
<img class="post" src="images/x-2026-01-06-hardwood-announced.png" alt="Jan 6: Hardwood announced, a perfect case for LLMs">

Note:
Read one sentence: "I think it's a perfect case for LLMs: a concise and
well-defined problem, authoritative spec, lots of training material out there."

And: "the idea is *not* to vibe code this."
...
<div class="pair">
  ...
  <img src="images/x-2026-01-28-flink-leak-invented-imports.png" alt="Jan 28: fixes unbounded state growth in a Flink job, then invents as imports for Java">
</div>

Note:
The upside is real. A race condition found and fixed. An unbounded state leak
in a Flink job, found and fixed — and a minute later it invents "as" imports
for Java.

Also good at: implementing a spec (encodings, page headers, Thrift), and
triaging failures — paste a hex dump, ask what's wrong, get the right answer.
...
## Both trades worked because a <em>loop</em> existed.

<span class="aside">Test vectors. My ears.</span>

Note:
Name the idea now. The SigV4 signer had a loop: AWS's test vectors. The note
trainer had a loop: I hear a wrong note instantly.

And it took knowing what to check to use either one.

Sets up the hinge: what if you can't just listen for it?
...
Call back to Jan 6: this is exactly why I picked the project. I chose the best
case on purpose.
...
how you build the <em>loops</em> you weren't handed.
...
Everything in this act is about moving that checking out of your head and into
something that runs.
...
The theme of this talk came out of the work, not the other way round.
...
while I was reading every diff — the geo bug among them.
...
1. <em>Correctness</em>: does it do the right thing?
2. <em>Speed</em>: how fast do you find out?
3. <em>Performance</em>: is it fast enough?
4. <em>Review</em>: what can no loop see?

Note:
The agenda for the rest of the act. Functional and non-functional, and the loop
itself.
...
About <em>72,000</em> predicate cells
...
<div class="pair">
  <img src="images/x-2026-01-20-sql-execution-plans.png" alt="Jan 20: LLMs are really good at comparing SQL execution plans">
  <img src="images/x-2026-01-23-flink-pipeline-diagram.png" alt="Jan 23: Claude Code draws a Flink pipeline as an ASCII diagram from its source code">
</div>

Note:
Before you can pin down behaviour, you have to know what the system does. Two
cheap wins: comparing a fast and a slow Postgres execution plan, and drawing a
Flink pipeline straight from its source.
...
Note:
February. A test gave an unexpected result. The agent didn't find out why. It
excluded the result and reported that everything passes. It slipped through,
despite me reviewing all the code at the time.
...
expected values, skipped tests — get read line by line, every time.
...
Call back to the end of act one: it took knowing what to check.
...
<span class="aside">What can no loop see?</span>
...
Note:
A breather before the review section. The joke from March, and the count from
my own session transcripts.
...
Note:
The agent calls it a day. You don't get to.
...
<img class="callback" src="images/x-2026-04-20-20h-refactoring.png" alt="Apr 20: Claude: that is about 20h of refactoring. 10 minutes later: Done.">
...
everything in September; the geo bug in May.
