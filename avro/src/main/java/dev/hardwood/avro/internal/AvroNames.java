...
final class AvroNames {
    ...
    TypeName typeName(SchemaNode node) {
        ...
        return new TypeName(localName(node), namespace);
    }
    ...
    /// The resolved local name of a node the resolver visited.
    ///
    /// A node the resolver never reached has no name to serve, and returning `null`
    /// would surface as a name-less Avro type built far from the cause. The resolver
    /// and [AvroSchemaConverter] classify groups in the same order, so a miss means
    /// the two have drifted apart, not that the schema is malformed.
